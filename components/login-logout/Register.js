...
function Register() {
  ...
  const APIRegister = () => {
    fetch("https://beverage-store7902.onrender.com/user/create-user", {
      method: "POST",
      headers: {
    ...
    <section className={styles["auth-form-container-register"]}>
      ...
      <form onSubmit={formik.handleSubmit} className={styles["register"]}>
          ...
          <div className={styles["phone-number-input-logo"]}>
            <div className={styles.phone}>
              <span className={styles["logo-number"]}>
                <Image className="phone-image" src={Logo} alt="Logo Việt Nam" />
                <span>+84</span>
              </span>
              <input
        ...
        <div style={{ marginTop: "10px" }}>
          <span className={styles["min-text"]}>
            Vui lòng nhấn &quot;Tiếp theo&quot; để nhận mã xác thực. Mã xác thực
            sẽ được gửi đến tin nhắn điện thoại của bạn
          </span>
        </div>
      ...
      <div className={styles["note"]}>
        <span style={{ display: "flex" }}>
          Đã có tài khoản?
          <Link href="/login">
            <p className={styles["custom-link"]} style={{ color: "orange" }}>
              Đăng nhập
            </p>
          </Link>
        </span>
