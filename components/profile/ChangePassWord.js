  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      newPass: Yup.string()
        ...
        .oneOf([Yup.ref("newPass"), null], "Mật khẩu mới và xác thực mật khẩu mới không đúng"),
...
useEffect(()=>{
    if(formik.errors.pass||formik.errors.newPass||formik.errors.confirmNewPass)
    {
      setIsValid(false);
    }
...
const ChangePassAPI = () => {
  ...
  else if(response.code==114&&formik.values.pass!=""&&formik.values.newPass!=""&&formik.values.confirmNewPass!=""){
    setTypeMess("error");
...
return (
    <section>
      ...
      <form onSubmit={formik.handleSubmit}>
          <div className={style["modal-container"]}>
              <div className={style["modal-content"]}>
                  <div className={style["modal-title"]}>
                      ...
                      <p className={style["modal-close"]} onClick={handleClose}>X</p>
                  </div>
          <div className={styles["pass-word"]}>
            ...
            <div className={styles["pass-word-input"]}>
              ...
              <div style={{height:"15px",position:"relative",marginBottom:"5px"}}>
                  {formik.errors.pass && (
                  <p style={{color:"#ff4d4f",fontSize:"14px",position:'absolute',bottom:"-20px",left:"0"}}> {formik.errors.pass} </p>)}
          </div>
       <div className={styles["pass-word"]}>
     ...
     <div style={{height:"10px",position:"relative",marginBottom:"5px"}}>
           {formik.errors.newPass && (
          <p style={{color:"#ff4d4f",fontSize:"14px",position:'absolute',bottom:"-20px",left:"0"}}> {formik.errors.newPass} </p>
          
        )}
     </div>
     <div className={styles["pass-word"]}>
          <span id={styles["medium-text-icon"]}>Nhập lại mật khẩu mới</span>
          <div className={styles["pass-word-input"]}>
         <span className={styles["pass-word-input-hide"]}>
         <input
          className={styles["pass-input"]}
     ...
     <div style={{height:"10px",position:"relative",marginBottom:"20px"}}>
           {formik.errors.confirmNewPass && (
          <p style={{color:"#ff4d4f",fontSize:"14px",position:'absolute',bottom:"-20px",left:"0"}}> {formik.errors.confirmNewPass} </p>
              ...
              <div className={style["btn-container"]}>
                  <button className={style["btn-cancel"]} onClick={handleClose}>Cancel</button>
                  <button className={style["btn-ok"]} disabled={!isValid} onClick={ChangePassAPI} >OK</button>
              </div>
          </div>
      </form>
    </section>
